...
{
#endif
static const enum ceu_str_join_null_action
{
	CEU_STR_JOIN_RETURN_NULL = 0,
	CEU_STR_JOIN_REGARD_AS_EMPTY_STR = 1,
	CEU_STR_JOIN_SKIP = 2
} ;
typedef enum ceu_str_join_null_action ceu_str_join_null_action_t;


inline static void ceu_die_with_retv(const char* reason, int retv)
{
	printf("Error occured! reason:\t%s", reason);
	exit(retv);
}

inline static void ceu_die(const char* reason)
{
 ...
 */
inline static void* ceu_smalloc(size_t size)
{
	void* m = malloc(size);
	if (m == NULL)
	{
...
* @return Allocated memory
*/
inline static void* ceu_srealloc(void* m, size_t size)
{
 ...
 * @return Allocated memory
 */
inline static void* ceu_scalloc(size_t count, size_t size)
{
...
inline static void ceu_free_non_null(void* m)
{
	if (m != NULL)
	{
		free(m);
...
inline static char* ceu_str_join(char* buff1, char* buff2)
{
...
inline static char* ceu_str_join_with_sep(char* sep, ceu_str_join_null_action_t nb, int count, ...)
{
	if (sep == NULL)
	{
		return NULL;
