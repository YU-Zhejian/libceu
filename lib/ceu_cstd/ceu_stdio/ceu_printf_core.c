...
static int ceu_printf_get_largest_integer_length() {
    // Should be 20
    int reti = 0;
 ...
 * @param sign
 * @param dest_length
 * @param ceu_printf_flag_minus the result of the conversion is left-justified within the field (by default it is right-justified)
 * @param padding_char
 */
static char *
ceu_printf_pad_blanks(char *converted_str, char arg_numeric_sign_char, size_t dest_length, bool ceu_printf_flag_minus,
                      char padding_char) {
    ...
    if (current_length >= dest_length) {
        return imm_str;
    } // FIXME: Wrong! If padding_char = ' ', will add sign and pad; '0' otherwise.
    ...
    if (ceu_printf_flag_minus) {
        ...
        for (int i = 0; i < dest_length - current_length; i++) {
...
ceu_printf_ret_t ceu_vsnprintf_core(
    ...
    u_int64_t arg_uint; // Value of the argument if it is unsigned int.
    int64_t arg_int; // Value of the argument if it is signed int.
    ...
    char arg_numeric_sign_char; // Sign of the numeric, should be ' ' (see flags), '+', '-' or '\0' (disabled)
            ...
            case CEU_PRINTF_PARSING_CONVENTION:
                switch (current_char) {
                    ...
                    case 'i':
                        ...
                        converted_padded_str = ceu_printf_pad_blanks(
                                converted_str,
                                arg_numeric_sign_char,
                                minimum_field_width,
                                pf.CEU_PRINTF_FLAG_MINUS,
                                padding_char
                        );


                        rett.current_buffer_position = ceu_printf_append_str_to_buff(buff, rett.current_buffer_position,
                                                                                     converted_padded_str);
                    ...
                    case 's':
                        ...
                        switch (length) {
                            ...
                            case -1:// Default char
                                rett.current_buffer_position = ceu_printf_append_str_to_buff(buff,
                                                                                             rett.current_buffer_position,
                                                                                             arg_str);
                                break;
                            default:
                                abort();
...
ceu_printf_ret_t ceu_snprintf_core(
    ...
    va_list(args);
    va_start(args, fmt);
    ceu_printf_ret_t retv = ceu_vsnprintf_core(
            buff, max_print_n_char, fmt,
            &args
    );
    va_end(args);
    return retv;
