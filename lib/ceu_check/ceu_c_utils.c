...
void* ceu_sreallocarray(void* m, size_t count, size_t size)
{
    void* new_m;
    CEU_ENSURE_NOT_NONE(m);
    new_m = reallocarray(m, count, size);
    if (new_m == NULL) {
        ceu_free_non_null(m);
        ceu_die_with_retv("ERR 12 -- Cannot allocate memory", 12);
    }
...
char* ceu_str_join_with_sep(char* sep, ceu_str_join_null_action_t nb, int count, ...)
{
    ...
    for (i = 0; i < count; ++i) {
        ...
        curr_buf = (char*)ceu_sreallocarray(curr_buf, strlen(new_intem_buf) + 1, sizeof(char));
        ...
        curr_buf = (char*)ceu_sreallocarray(curr_buf, strlen(new_intem_buf2) + 1, sizeof(char));
